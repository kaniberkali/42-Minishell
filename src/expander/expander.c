...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/06/01 15:45:04 by akaniber          #+#    #+#             */
/*   Updated: 2023/10/15 13:38:10 by akaniber         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_env(t_shell *shell, t_list *data, char *tmp, int i)
{
	t_env	*y;
	char	*x;
	t_list	*shell_data;
...
void	_get_env_to_lexlist(t_shell *shell, char **tmp, int i, t_list **data)
{
	free(*tmp);
...
void	get_env_to_lexlist(t_shell *shell, t_lexer **lex_list)
{
	t_list	*data;
	char	*tmp;
	char	*tmp2;
	...
	while (data)
	{
		...
		while (tmp[i])
		{
			_get_env_to_lexlist(shell, &tmp, i, &data);
			if (tmp[i] == '$' && tmp[0] != '\'')
			{
				check_env(shell, data, tmp, i);
...
void	expander(t_shell *shell)
{
	if (ft_strcmp(shell->cmd, ""))
		get_env_to_lexlist(shell, &(shell->lex_list));
}
